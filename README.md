...
After checkout, let Unity finish importing `Assets/_Game/Art/Kenney` and `Assets/_Game/Audio/Kenney`, then run **Core Guard → Configure Project** once. Open `Assets/_Game/Scenes/Main.unity` and press Play. The game opens on a full-screen home screen; use **Start** to enter the arena and **Settings** to toggle SFX/BGM independently. The short loading transition is followed by the 3–2–1 countdown. Move with **WASD or arrow keys**, aim with the mouse, select Bullet/Rocket/Mine with **1/2/3**, fire with the left mouse button, activate Shield/EMP with **0/E**, and interact with X/Y/Z. **Esc** pauses/resumes. The result panel's **Retry** button or **R** resets the run after a win/loss; **F1** opens the repeatable Demo Mode panel.
