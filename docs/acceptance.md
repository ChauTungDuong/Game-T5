...
| R09 | A state changes | Live HP, Armor, Coins, weapon/cooldown HUD | HUD test passes; enemy projectile hits now reduce HP and Armor immediately, with red player health-bar feedback | PASS automated |
...
1. Run `Core Guard → Configure Project`, open `Assets/_Game/Scenes/Main.unity`, press Play; confirm the full-screen entry screen hides the arena, open Settings to toggle SFX/BGM independently, then click Start and confirm loading followed by the centered 3–2–1 image countdown.
2. Move with WASD/arrows and aim with the mouse. Press 1/2/3 and confirm the HUD weapon label changes; press 0 for Shield and E for EMP; hold left mouse for Bullet and click for Rocket/Mine.
...
4. Press 0 before an enemy shot and confirm Shield absorbs up to three hits. Press E near enemies and confirm only nearby enemies stun for about two seconds.
