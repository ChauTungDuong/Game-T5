...
namespace CoreGuard.Tests.PlayMode
{
    public sealed class PresentationInputTests : InputTestFixture
    {
        ...
        [UnityTest] public IEnumerator Hud_ShowsLiveValuesAndButtonsDriveSingleSceneFlow()
        {
            ...
            hud.LoadingPanel = Make<Transform>("Loading panel").gameObject;
            hud.LoadingProgress = Make<Image>("Loading progress");
            ...
            hud.Bind();
            Assert.That(hud.StatsText.text, Is.EqualTo("HP 100/100  •  ARM 50/50  •  C 0"));
            Assert.That(hud.CoreText.text, Is.EqualTo("CORE 100/100"));
            Assert.That(hud.TimerText.text, Is.EqualTo("90.0s"));
            Assert.That(hud.WeaponText.text, Is.EqualTo("BULLET"));
            Assert.That(hud.CooldownsText.text, Does.Contain("SH READY"));
            ...
            Assert.That(hud.LoadingPanel.activeSelf, Is.True);
            ...
            Assert.That(hud.WeaponText.text, Is.EqualTo("ROCKET"));
            s.Weapon.Select(WeaponKind.Bullet);
            s.Player.ApplyDamage(60); s.Player.AddCoins(7); s.Core.ApplyDamage(20); s.Advance(.5f);
            Assert.That(hud.StatsText.text, Does.Contain("HP 90/100"));
            Assert.That(hud.StatsText.text, Does.Contain("ARM 0/50"));
            Assert.That(hud.StatsText.text, Does.Contain("C 7"));
            ...
            Assert.That(hud.CooldownsText.text, Does.Contain("SH 3/3"));
